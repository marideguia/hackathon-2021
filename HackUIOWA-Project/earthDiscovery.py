...
country_list = ["USA", "UK", "China", "Canada", "Japan", "France", "Korea", "Mexico", "South Africa", "Spain", " "]
...
# Generate valid spawn coordinates
# def validSpawn(i):
#     xspawn = random.randint(0,710)
#     yspawn = random.randint(50,150)
#     for j in range(num_of_flags):
#         distance = math.sqrt((math.pow(xspawn - flagX[j], 2)) + (math.pow(yspawn - flagY[j], 2)))
#         while distance < 80:
#             xspawn = random.randint(0, 710)
#             yspawn = random.randint(50, 150)
#             distance = math.sqrt((math.pow(xspawn - flagX[j], 2)) + (math.pow(yspawn - flagY[j], 2)))
...
# Check flag collision: pass flag[i] coordinates, pass flag[j] coordinates, if collide, then change direction
def flagCollision(flagX, flagY, flag2X, flag2Y, i, j):
    distance = math.sqrt((math.pow(flagX - flag2X, 2)) + (math.pow(flagY - flag2Y, 2)))
    if distance < 80:
        flagX_change[i] = -flagX_change[i]
        flagY_change[i] = -flagY_change[i]
        flagX_change[j] = -flagX_change[j]
        flagY_change[j] = -flagY_change[j]

# Populate arrays with flag attributes
for i in range(num_of_flags):
    flagX.append(random.randint(0, 710))
    flagY.append(random.randint(50, 150))
...
flagImg.append(pygame.image.load('france.png'))
flagImg.append(pygame.image.load('korea.png'))
flagImg.append(pygame.image.load('mexico.png'))
flagImg.append(pygame.image.load('south-africa.png'))
flagImg.append(pygame.image.load('spain.png'))
...
while running:
    ...
    for i in range(num_of_flags):
        ...
        # Check for collision between flags
        for j in range(num_of_flags):
            if j != i:
                flagCollision(flagX[i], flagY[i], flagX[j], flagY[j], i, j)

        # Check for collision between star and flags
